...
def get_products_by_category(category_id, user=None):
    if user and user.is_authenticated and user.has_perm("catalog.can_unpublish_product"):
        ...
        return cache.get_or_set(
            cache_key,
            lambda: list(Product.objects.filter(category_id=category_id, status="published")),
            timeout=300
        )
